...
split_loop_frame(__cilkrts_stack_frame *frame_to_steal, __cilkrts_worker *w, __cilkrts_loop_frame **res_lf) {
    if (__cilkrts_is_loop(frame_to_steal)) {
        ...
        cilkrts_alert(LOOP | ALERT_STEAL, w, "(split_loop_frame) Splitting frame %p [%lu:%lu] on victim %d in two (new_lf=%p)!\n", lf, start, lf->end, lf->sf.worker->self, new_lf);
        ...
        cilkrts_alert(LOOP | ALERT_STEAL, w, "(split_loop_frame) Restoring head: %d, mid=%lu\n", retractExc, mid);
...
void sync_loop_frame(__cilkrts_worker *w, Closure *t) {
    if (!t->most_original_loop_frame) {
        ...
        if (t->fiber) {
            cilkrts_alert(SYNC | ALERT_LOOP, w, "(sync_loop_frame) Scenario 2.a), closure %p\n", t);
            // in case a), we still haven't freed our fiber
            CILK_ASSERT(w, w->current_stack_frame == t->frame);
        } else {
            cilkrts_alert(SYNC | ALERT_LOOP,w,  "(sync_loop_frame) Scenario 2.b), closure %p\n", t);
