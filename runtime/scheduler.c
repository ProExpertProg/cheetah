...
static void setup_for_sync(__cilkrts_worker *w, Closure *t) {
    __cilkrts_alert(ALERT_SYNC, "[%d]: (setup_for_sync) closure %p\n", w->self, t);
...
Closure * Closure_return(__cilkrts_worker *const w, Closure *child) {
    ...
    if(child->left_sib || parent->fiber_child) {
        ...
        if (child->most_original_loop_frame) {
            ...
            __cilkrts_alert(ALERT_LOOP | ALERT_RETURN,
                            "[%d]: Getting most_orig_lf %p from child %p for parent %p\n",
                            w->self, child->most_original_loop_frame, child, parent);
        }
    }
    ...
    if( w->head > w->tail ) {
        ...
        if (isLoop) {
            ...
            if (__cilkrts_is_dynamic(w->current_stack_frame))
                cilk_internal_free(w, w->current_stack_frame, sizeof(__cilkrts_loop_frame));
            else {
...
void finish_promote(__cilkrts_worker *const w, 
    ...
    // if parent->frame was split but not stolen, it is now stolen
        ...
        switch (cl->status) {
            ...
            case CLOSURE_RUNNING:
                ...
                if(do_dekker_on(w, victim_w, cl)) {
                    ...
                    // now we're absolutely certain we won't fail
